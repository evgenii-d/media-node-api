...
def list_player_instances() -> list[ConfigSchemaOut]:
    files = get_dir_files(AppDir.PLAYER_CONFIGS.value)
    if not files:
        raise HTTPException(404, "No player instances found")

    instances = []
    for file in files:
        data = ConfigManager(AppDir.PLAYER_CONFIGS.value/file).load_section()
...
def create_player_instance(data: ConfigSchemaIn) -> ConfigSchemaOut:
    ...
    for file in AppDir.PLAYER_CONFIGS.value.iterdir():
    ...
    new_config_path = AppDir.PLAYER_CONFIGS.value/f"{uuid4().hex}.ini"
